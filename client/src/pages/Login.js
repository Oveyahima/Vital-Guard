...
const Login = () => {
  ...
  const login = async (isDoctor) => {
    Object.keys(cookies).forEach(cookieName => {
      removeCookie(cookieName, { path: '/' });
    });

    let userType = isDoctor ? 'doctor' : 'patient';
    const apiUrl = `http://localhost:8050/api/doctors/login`; // Adjust the URL to match your backend route
      ...
      <div className="bg-gray-100 flex rounded-2xl shadow-lg max-w-3xl p-5 items-center bg-white/20 z-10 backdrop-filter backdrop-blur-lg shadow-lg ">
        <div className="md:w-1/2 px-8 md:px-16">
          ...
          <form action="" className="flex flex-col gap-4">
            <input className="p-2 mt-8 rounded-xl border" type="email" name="email" id="email" placeholder="Email" onChange={(e) => handle(e)} />
